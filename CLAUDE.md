...
- **`data/verse-index.json` is generated, and committed.** `node tools/build-verse-index.js` rebuilds it from `data/verses.json` — regenerate and commit it whenever `verses.json` changes, or the CMS verse picker at `/admin/` goes stale. It exists because Sveltia reads the repo through GitHub's GraphQL API, which truncates blobs over 512,000 bytes; `verses.json` is ~732 KB (Devanagari is 3 bytes/char) and arrives cut mid-string. The index is English-only (`id`, `ref`, 120-char `en`) and ~128 KB. Never point the CMS at `verses.json` itself.
